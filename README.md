# 🌐 Raghav Gupta – Web3 & Blockchain Developer Portfolio

This repository contains my personal portfolio website showcasing my skills, projects, and experience in **Web3 and Blockchain development**.

---

## 🚀 Live Portfolio
🔗 **View here:** https://raghavguptarts321.github.io/Portfolio/

---

## 🧑‍💻 About Me
I am a blockchain-focused software developer with hands-on experience in building decentralized applications using **Solidity, React, and Ethereum**.  
I enjoy designing secure, transparent, and scalable Web3 systems.

---

## 🛠️ Tech Stack
- **Frontend:** React, Vite, Tailwind CSS
- **Blockchain:** Solidity, Ethereum (Sepolia)
- **Libraries:** Ethers.js
- **Tools:** Git, Hardhat, MetaMask

---

## 📌 Featured Projects

### 🔹 Web3 Ethereum Transfer DApp
- Wallet-based ETH transfer using MetaMask
- Transaction history with Etherscan links
- Responsive UI built with Tailwind CSS

### 🔹 X-Genesis – Edu NFT Platform
- Soulbound NFTs for verified academic credentials
- AI + institute-level verification
- Recruiter search & candidate discovery

### 🔹 Decentralized Voting DApp
- One vote per wallet
- On-chain vote storage
- Real-time voting results

---

## 📫 Contact
- **Email:** rg0257245@gmail.com
- **LinkedIn:** https://linkedin.com/in/raghav-gupta-381690284/

---

⭐ If you like my work, feel free to star this repository!
